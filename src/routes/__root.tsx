    ...
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/85 backdrop-blur">
      ...
      >
        ...
        <aside
          ...
          className={`absolute right-0 top-0 h-full w-[85vw] max-w-sm bg-background shadow-2xl transition-transform duration-300 ease-out overflow-hidden ${open ? "translate-x-0" : "translate-x-full"} flex flex-col`}
        >
          ...
          <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-5 py-4">
            ...
            <button
              ...
              onClick={() => setOpen(false)}
              className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-border text-foreground/80 transition hover:bg-secondary hover:text-[color:var(--maroon)]"
            >
              ...
              {NAV.map((n) => (
                <li key={n.to}>
                  <Link
                    to={n.to}
                    onClick={() => setOpen(false)}
                    className="block truncate rounded-lg px-4 py-3 text-[15px] font-medium text-foreground/85 transition hover:bg-secondary hover:text-[color:var(--maroon)]"
                    activeProps={{ className: "block truncate rounded-lg px-4 py-3 text-[15px] font-semibold text-[color:var(--maroon)] bg-secondary" }}
                    activeOptions={{ exact: n.to === "/" }}
                  >
                    {n.label}
                  </Link>
          ...
          <div className="flex shrink-0 gap-2 border-t border-border p-4">
            <a href={WA_LINK} target="_blank" rel="noreferrer" onClick={() => setOpen(false)} className="btn-whatsapp !py-2 !px-3 text-sm flex-1 justify-center"><WhatsAppIcon /> WhatsApp</a>
            <a href={waMessageLink(WA_PRESETS.donate())} target="_blank" rel="noreferrer" onClick={() => setOpen(false)} className="btn-gold !py-2 !px-3 text-sm flex-1 text-center">Donate</a>
          </div>
        </aside>
      </div>
    </header>
